  ...
  try {
    ...
    if (req.query.page && !isNaN(parseInt(req.query.page))) {
      ...
      return res.status(200).json({
        info: {
          totalJuegos: numJuegos,
          page: page,
          limit: limit,
          next:
            numPages >= page + 1
              ? `/juegos?page=${page + 1}&limit=${limit}`
              : null,
          prev: page != 1 ? `/juegos?page=${page - 1}&limit=${limit}` : null,
        },
      ...
      const allJuegosDB = await Juego.find().limit(10);
      //---- Recogemos el número total de juegos
      const numJuegos = await Juego.countDocuments();
      ...
      return res.status(200).json({
        info: {
          ...
          page: 1,
          limit: 10,
          next: numJuegos > 10 ? `/juegos?page=2&limit=10` : null,
          prev: null,
        },
    ...
    return next(
      "No se han podido solicitar los juegos a la base de datos",
      error
    );
  }
};
...
const getJuegoById = async (req, res, next) => {
  ...
  try {
    ...
    if (!juegoDB) {
      return next(
        "No se encuentra la unidad de juego a la que usted está tratando de dar búsqueda."
      );
    }
